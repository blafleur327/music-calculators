...
LibraryItem('Prime Form',`The tightest packed to the left between the normal order or its inversion transposed to 0. The prime form is highly abstract, and as such is typically notated with parenthesis (). The prime form is often used to reference or confirm membership within an entire set-class.`);
...
LibraryItem('Powerset',`The set of all subsets of a given collection. Mathematically, the powerset of a set P(<em>S</em>) is of the size 2<sup><em>n</em></sup>, which becomes a generation issue for larger supersets. Musically, the "List of Set-Classes" is the powerset of the 12-tone chromatic universe where every entry is reduced to prime form and duplicates are eliminated.`);
