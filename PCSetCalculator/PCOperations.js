...
function DrawingManager (parent = document.querySelector(`#drawing`)) {
    ...
    */
    ...
    this.information = () => {
        ...
        par.innerHTML = '';
        let included = ['Normal','Prime','ICV','IV','DoS','DF','WF','CV','ME'];
            ...
            included.forEach(spec => {
                ...
                switch (true) {
                    ...
                    case (spec == 'CV'):
                        ...
                        break;
                    case (spec == 'WF'):
                        let tem = setRep.wellFormed();
                        console.table(tem)
                        c.textContent = 'Well-Formed';
                        for (let x = 0; x < 3; x++) {
                            let z = document.createElement('div');
                            z.classList.add('hov');
                            if (x == 0) {
                                ...
                                z.textContent = `Generators: ${tem['Well-Formed']? setRep.wellFormed()['Generators'].toString() : 'N/A'}`;
                                z.dataset.tooltip = `The OPCI's that can generate this collection through recursive application.`;
                            }
                            d.append(z);
                        }
                        c.dataset.tooltip = `A Well-Formed collection can be generated by recursive application of a single interval. This collection is non-degenerate if this interval is coprime to the chromatic universe, and is degenerate otherwise.`;
                        break;
                }
                big.appendChild(c);
                big.appendChild(d);
