...
const Serialism = {
    ...
    /**
     * Creates Stravinskian Rotational Array out of an input.
     * @param {array} row 
     * @param {int} universe
     * @param {boolean} transpose wether to transpose the rotations.
     * @returns 2D array 
     */
    rotationalArray: function (row,universe = 12,transpose = false) {
        let rots = [];
        let startPitch = row[0];
        for (let a = 0; a < row.length; a++) {
            let temp = ArrayMethods.singleRotate(row,a);
            transpose? rots.push(temp.map(x => Serialism.modulo(x-(temp[0]-startPitch),universe))) : rots.push(temp);
        }
        return rots;
    },
    /**
    ...
    rowDictionary: function(row,universe = 12) {
        ...
        for (let a = 0; a < universe; a++) {
            ...
            dict[`RP${a}`] = Serialism.singleRowForm(row,`RP${a}`);
...
/**
 * An object that manages the RA.
 * @param {string} parent id of parent element. 
 */
function DoubleMatrix(parent) {
    /**
     * Object that stores information for RA structures.
     */
    this.data = {
        'row': null,
        'rowForm': null,
     ...
     * @param {int} universe 
     * @param {array} row 
     * @param {string} form {P,RP,I,RI}
     * @param {boolean} transpose 
     */
    this.construct = (universe = this.data['universe'],row = this.data['row'],form = this.data['rowForm'],transpose = this.data['arrayType']) => {
        this.data['alphaCells'] = [];
        this.data['betaCells'] = [];
        this.data['row'] = row;
        this.data['universe'] = universe;
        this.data['rowForm'] = form;
        // form = form? form : `P${row[0]}`;
        // this.data['form'] = form;
        console.log(`construct called!`);
        /**
         * Clear all previous elements inside of parent;
         */
        document.querySelectorAll(`#${parent} > *`).forEach(elem => {
            elem.remove();
        })
        let bigParent = document.querySelector(`#${parent}`);
        bigParent.classList.add('double');
        let a = document.createElement('div');
        let b = document.createElement('div');
        a.id = 'alpha';
        b.id = 'beta';
        a.classList.add('miniMatrix');
        b.classList.add('miniMatrix');
        bigParent.append(a,b);
        let modified = Serialism.singleRowForm(row,form,this.data['universe']);
        this.data['alpha'] = Serialism.rotationalArray(modified.slice(0,row.length/2),this.data['universe'],transpose);
        this.data['beta'] = Serialism.rotationalArray(modified.slice(row.length/2),this.data['universe'],transpose);
        for (let a = 0; a < 2; a++) {
            let par = a == 0? document.querySelector('#alpha') : document.querySelector('#beta');
            let title = document.createElement('h4');
            let greek = null;
            if (a == 0) {
                greek = this.data['arrayType'] == true? 'γ' : 'α';
            }
            else {
                greek = this.data['arrayType'] == true? 'δ' : 'β';
            }
            let prefix = ['','singleton','dyad','tri','tetra','penta','hexa','hepta','octa','nona','deca','undeca','dodeca'];
            title.textContent = `${prefix[row.length/2]}${row.length/2 > 2? 'chord' : ''} ${greek}:`;
            par.append(title);
            let curr = this.data[`${a == 0? 'alpha' : 'beta'}`];
            for (let b = 0; b < curr.length; b++) {
                let row = curr[b];
                let r = document.createElement('div');
                r.classList.add('row');
                let lab = document.createElement('div');
                lab.classList.add('label','cell');
                lab.textContent = ['','I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII','XVIII','XIX','XX'][b];
                // r['data-tooltip'] = `Rotation: ${lab.textContent}`;
                r.append(lab);
                let repRow = [];
                row.forEach(cell => {
                ...
                this.data[a == 0? 'alphaCells' : 'betaCells'].push(repRow);
                par.append(r);
            }
            /**
             * Set up the transposed matrices
             */
            if (a == 0) {
                this.data['alphaT'] = ArrayMethods.flipDimensions(this.data['alpha']);
                this.data['alphaTCells'] = ArrayMethods.flipDimensions(this.data['alphaCells']);
            }
            else {
                this.data['betaT'] = ArrayMethods.flipDimensions(this.data['beta']);
                this.data['betaTCells'] = ArrayMethods.flipDimensions(this.data['betaCells']);
            }
        }
    }
    /**
     * Searches both matrices for adjacent elements.
     * @param {array} query 
     */
    this.search = (query) => {
        console.log(`Searching RAs for {${query}}`)
        //Horizontal
        document.querySelectorAll('.find,.vertFind,.both').forEach(entry => {
            entry.classList.remove('find');
        })
        for (let a = 0; a < 2; a++) {
            for (let b = 0; b < this.data['row'].length/2; b++) {
                let t = this.data[a == 0? 'alpha' : 'beta'];
                let vt = this.data[a == 0? 'alphaT' : 'betaT'];
                let hRef = this.data[a == 0? 'alphaCells' : 'betaCells'];
                let vRef = this.data[a == 0? 'alphaTCells' : 'betaTCells'];
                let hRes = ArrayMethods.adjacentIndices(t[b],query).filter(x => x.length > 0);
                let vRes = ArrayMethods.adjacentIndices(vt[b],query).filter(x => x.length > 0);
                vRes.length > 0? console.log(vRes) : null;
                hRes.forEach(adj => {
                    console.log(`Found in ${a == 0? 'alpha' : 'beta'}-row ${b}-cells ${adj[0]}-${adj[1]}`);
                    for (let c = adj[0]; c <= adj[1]; c++) {
                        console.log(hRef[b][c]);
                        hRef[b][c].classList.add('find');
                    }
                })
                vRes.forEach(adj => {
                    console.log(`Found in ${a == 0? 'alpha' : 'beta'}-column ${b}-cells ${adj[0]}-${adj[1]}`);
                    for (let c = adj[0]; c <= adj[1]; c++) {
                        console.log(vRef[b][c]);
                        vRef[b][c].classList.add('find');
                    }
                })
            }
        }
...
document.addEventListener('DOMContentLoaded',() => {
    console.log('Loaded.');
    Manager = new DoubleMatrix('storage');
    Drop = new MyDropdown('dropContainer','Row-Form');
    arrayType = new MyDropdown('typeDrop','Array-Type');//Makes copy upon hitting enter... 
    document.querySelectorAll('input').forEach(inp => {
        inp.addEventListener('keydown',(event) => {
            if (event.key == 'Enter') {
                if (inp.id == 'univ') {
                    Manager.data['universe'] = parseInt(inp.value);
                    console.log(`Universe: ${inp.value}`);
                }
                else if (inp.id == 'trow') {
                    Manager.data['row'] = inp.value.match(/[0-9]+/ig).map(x => parseInt(x));
                    console.log(`Row: <${inp.value}>`);
                }
                else if (inp.id == 'searchbar') {
                    console.log(`Search for: [${inp.value}]`);
                    Manager.search(inp.value.match(/[0-9]+/ig).map(x => parseInt(x)));
                }
                //
                if (Manager.data['universe'] !== null && Manager.data['row'] !== null) {
                    arrayType.addMethod((value) => {
                        Manager.data['arrayType'] = value;
                        Manager.construct(Manager.data['universe'], Manager.data['row'],Manager.data['rowForm'],value);//????
                    });
                    Drop.addMethod((value) => {
                        Manager.data['rowForm'] = value;
                        Manager.construct(Manager.construct());
                    });
                    Drop.removeOptions();
                    arrayType.removeOptions();
                    Object.entries(Serialism.rowDictionary(Manager.data['row'],Manager.data['universe'])).forEach(([key,value]) => {
                        Drop.addOption(key,key,`Change display to ${key} of input.`);
                    });
                    arrayType.addOption('α/β',false,'Create pure rotational array, no transposition.');
                    arrayType.addOption('γ/δ',true,'Create rotational array with each rotation transposed to starting PC.');
                    Drop.construct();  
                    arrayType.construct();               
                }
            }
