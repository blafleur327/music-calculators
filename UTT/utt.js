...
function Drawing (parent) {
    ...
    this.drawCycle = (start,...utts) => {
        ...
        for (let a = 0; a < points.length; a++) {
            ...
            b.self['node']['data-tooltip'] = `(${utts.map(x => x instanceof UTT? x.stringFormat : x)})<sup>${Math.floor(a/STORE.chain.length)}</sup> `;
            ...
            x.center(...offPoints[(a*2)+1]);//Add tooltip?//x is the UTT display
            x['node']['data-tooltip'] = ref? ref.tooltip : undefined;
...
const mouseTracking = () => {
    ...
    document.addEventListener('mouseover',(element) => {    //Whole document allows tooltips!
        ...
        console.log(element.target.parentNode.classList)
        if (element.target.parentNode.classList.contains('transform') && !element.target.parentNode.parentNode.classList.contains('MyNode')) {    //Catch text
            console.log(`THIS IS A TRANSFORM!`);
            message = element.target.parentNode.closest('.transform')['data-tooltip'];
        }
        else if (element.target.parentNode.classList.contains('MyNode') || element.target.parentNode.parentNode.classList.contains('MyNode')) {   
            console.log('Trigger Condition 2.');
            message = element.target.closest('.MyNode')['data-tooltip'];
        }
        else {
